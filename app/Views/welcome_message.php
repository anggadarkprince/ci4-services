...
<script>
    const SUBSCRIBE_PAYMENT = 'payment-notification';
    const EVENT_INVOICE_PUBLISHED = 'invoice-published';

    function displayNotification(title, message, url = 'http://warehouse-ci4.transcon-indonesia.local') {
        let options = {
            body: message,
            icon: 'https://transcon-indonesia.com/training/assets/dist/img/icon.png',
        };
    ...
    if ('Notification' in window) {
        if (Notification.permission !== "granted") {
            ...
            let channelSyllabus = pusher.subscribe(`${SUBSCRIBE_PAYMENT}.1`);
            channelSyllabus.bind(EVENT_INVOICE_PUBLISHED, function (data) {
                displayNotification('Invoice Published', data.message, data.url);
            });
        }
    } else {
